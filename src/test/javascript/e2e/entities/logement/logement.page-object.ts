...
export class LogementUpdatePage {
  ...
  typeDePieceSelect = element(by.id('field_typeDePiece'));
  ...
  async setTypeDePieceSelect(typeDePiece: string): Promise<void> {
    await this.typeDePieceSelect.sendKeys(typeDePiece);
  }

  async getTypeDePieceSelect(): Promise<string> {
    return await this.typeDePieceSelect.element(by.css('option:checked')).getText();
  }

  async typeDePieceSelectLastOption(): Promise<void> {
    await this.typeDePieceSelect.all(by.tagName('option')).last().click();
  }
