...
export class PaiementUpdatePage {
  ...
  typePaiementSelect = element(by.id('field_typePaiement'));
  ...
  async setTypePaiementSelect(typePaiement: string): Promise<void> {
    await this.typePaiementSelect.sendKeys(typePaiement);
  }

  async getTypePaiementSelect(): Promise<string> {
    return await this.typePaiementSelect.element(by.css('option:checked')).getText();
  }

  async typePaiementSelectLastOption(): Promise<void> {
    await this.typePaiementSelect.all(by.tagName('option')).last().click();
  }
